/**
 * HTML entities decoding table from https://html.spec.whatwg.org/entities.json
 */
...
/**
 * Convert HTML character references to their Unicode representation
...
function HTMLCharDecode(str) {
	str = str.replace(/&[a-zA-Z]+?;/g, substr => {
		return translationTable[substr].characters;
	});
	str = str.replace(/&#[xX][0-9a-fA-F]+?;/g, substr => {
		return String.fromCodePoint(parseInt(substr.slice(3, -1), 16));
	});
	str = str.replace(/&#[0-9]+?;/g, substr => {
		return String.fromCodePoint(parseInt(substr.slice(2, -1)));
	});
	return str;
}

module.exports = HTMLCharDecode;
