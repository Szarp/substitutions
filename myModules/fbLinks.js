...
*/
    ...
    function messengerApi(userId){
        return 'https://graph.facebook.com/v2.9/'+userId+'?fields=first_name,last_name,profile_pic&access_token='+config.pageToken;
        
    }
    function loginAttempt(permissions){ //opens a facebook dialog window, returns code 
        var redirect='/redirect';
        return  'https://www.facebook.com/v2.9/dialog/oauth?'+'client_id='+config.clientId+'&redirect_uri='+config.url+redirect+'&scope='+permissions;
        /*
    ...
    function userAccesToken(code){ //exchange code for Token
        var redirect='/redirect';
        return 'https://graph.facebook.com/v2.9/oauth/access_token?'+ 'client_id='+config.clientId+'&redirect_uri='+config.url+redirect+'&client_secret='+config.appSecret+'&code='+code;
    ...
    function tokenInfo(token){ //information about token 
        return 'https://graph.facebook.com/v2.9/debug_token?input_token='+token+'&access_token='+config.appToken;
        /*
             GET graph.facebook.com/v2.9/debug_token?
             input_token={token-to-inspect}
             &access_token={app-token-or-admin-token}
        */
    }
function userInfo(token,params){  //information about user
    return 'https://graph.facebook.com/v2.9/me?access_token='+token+'&debug=all&fields='+params+'&format=json&method=get&pretty=0&suppress_http_code=1';
    ...
    format=json&  
    method=get&   
    pretty=0&  //return JSON in one line (not human-readable)
    suppress_http_code=1  //don't know what is it
    
    */
}
function whiteListing(){
    return "https://graph.facebook.com/v2.9/me/thread_settings?access_token="+config.pageToken;
    /*
...
function notification(id,message,redirect){ //send notification to user
    return 'https://graph.facebook.com/v2.9/'+id+'/notifications?access_token='+config.appToken+'&href='+redirect+'&template='+message;
     /*
